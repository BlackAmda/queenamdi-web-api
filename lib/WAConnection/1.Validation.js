...
class WAConnection extends _0_Base_1.WAConnection {
    ...
    async authenticate(reconnect) {
        ...
        if (response.status && response.status !== 200) {
            throw new Constants_1.AmdiWebError(`Unexpected error in login`, { response, status: response.status });
        }
    ...
    validateNewConnection(json) {
        ...
        if (!hmac.equals(secret.slice(32, 64))) {
            ...
            throw new Constants_1.AmdiWebError('HMAC validation failed', json);
        }
