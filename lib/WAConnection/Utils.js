...
exports.Browsers = {
    ...
    amdiweb: browser => ['AmdiWeb', browser, '3.0'],
...
function decryptWA(message, macKey, encKey, decoder, fromMe = false) {
    ...
    if (commaIndex < 0)
        throw new Constants_1.AmdiWebError('invalid message', { message }); // if there was no comma, then this message must be not be valid
    ...
    if (data.length > 0) {
        if (typeof data === 'string') {
            ...
            if (!macKey || !encKey) {
                throw new Constants_1.AmdiWebError('recieved encrypted buffer when auth creds unavailable', { message });
            }
            /*
            ...
            */
            ...
            if (checksum.equals(computedChecksum)) {
                ...
                throw new Constants_1.AmdiWebError('checksum failed', {
...
const getGotStream = async (url, options = {}) => {
    ...
    await new Promise((resolve, reject) => {
        ...
        fetched.once('response', ({ statusCode: status }) => {
            if (status >= 400) {
                reject(new Constants_1.AmdiWebError('Invalid code (' + status + ') returned', { status }));
            }
            else {
                resolve(undefined);
            }
...
async function decryptMediaMessageBuffer(message) {
    ...
    */
    ...
    if (!type) {
        throw new Constants_1.AmdiWebError('unknown message type', message);
    }
    if (type === Constants_1.MessageType.text || type === Constants_1.MessageType.extendedText) {
        throw new Constants_1.AmdiWebError('cannot decode text message', message);
    }
    ...
    if (message.productMessage) {
        ...
        if (!product)
            throw new Constants_1.AmdiWebError('product has no image', message);
