...
class WAConnection extends _6_MessagesSend_1.WAConnection {
    ...
    generateForwardMessageContent(message, forceForward = false) {
        ...
        if (!content)
            throw new Constants_1.AmdiWebError('no content in message', { status: 400 });
    ...
    async modifyChat(jid, type, arg) {
        ...
        if (type === Constants_1.ChatModification.mute && !arg) {
            throw new Constants_1.AmdiWebError('duration must be set to the timestamp of the time of pinning/unpinning of the chat', { status: 400 });
        }
