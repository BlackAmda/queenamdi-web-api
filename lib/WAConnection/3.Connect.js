...
class WAConnection extends _1_Validation_1.WAConnection {
    ...
    async connect() {
        ...
        if (this.state !== 'close') {
            throw new Constants_1.AmdiWebError('cannot connect when state=' + this.state, { status: 409 });
        }
        ...
        while (this.state === 'connecting') {
            ...
            try {
                ...
                const loggedOut = error instanceof Constants_1.AmdiWebError && Constants_1.UNAUTHORIZED_CODES.includes(error.status);
