...
class WAConnection extends _5_User_1.WAConnection {
        ...
        switch (type) {
            ...
            case Constants_1.MessageType.extendedText:
                ...
                if ('text' in message) {
                    ...
                    throw new Constants_1.AmdiWebError('message needs to be a string or object with property \'text\'', message);
                }
                break;
    ...
    async updateMediaMessage(message) {
        ...
        if (!content)
            throw new Constants_1.AmdiWebError(`given message ${message.key.id} is not a media message`, message);
    ...
    async downloadMediaMessage(message, type = 'buffer') {
        ...
        if (!mContent)
            throw new Constants_1.AmdiWebError('No message present', { status: 400 });
        ...
        try {
            ...
            if (error instanceof Constants_1.AmdiWebError && error.status === 404) { // media needs to be updated
